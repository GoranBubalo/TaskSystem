...
class TaskRepository:
    def create_task(self, db: Session, task: TaskCreate, user_id: int) -> Task:
        """Create a new task"""
        db_task = Task(
            title=task.title,
    ...
    def update_task(self, db: Session, task_id: int, task_update: TaskUpdate) -> Optional[Task]:
        """Update a task"""
        ...
        if 'is_completed' in update_data and update_data['is_completed'] != db_task.is_completed:
            ...
            # Remove these from update_data to avoid overwriting twice
    ...
    def get_subtasks(self, db: Session, parent_id: int) -> List[Task]:
